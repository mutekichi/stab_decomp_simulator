...
from .toffoli_state import construct_toffoli_tensor_state
...
def qiskit_circuit_to_t_decomposed_stab_decomp_state(qc: QuantumCircuit) -> StabilizerDecomposedState:
    """
...
def qiskit_circuit_to_toffoli_decomposed_stab_decomp_state(qc: QuantumCircuit) -> StabilizerDecomposedState:
    """
    Given a Qiskit circuit C composed of only Clifford+Toffoli gates,
    returns the state |ψ⟩ = C|0⟩ as a stabilizer-decomposed state.
    """
    num_qubits_orig = qc.num_qubits
    num_toffoli_gates = 0
    ...
    # First pass to parse the circuit and separate Toffoli gates from Cliffords
    for instruction in qc.data:
        instr = instruction.operation
        qregs = instruction.qubits
        qubits = [q._index for q in qregs]
        name = instr.name.lower()
        if name == "ccx":
            ancilla_idx = num_qubits_orig + 3 * num_toffoli_gates

            clifford_ops.append(("cx", [qubits[0], ancilla_idx]))
            clifford_ops.append(("cx", [qubits[1], ancilla_idx + 1]))
            clifford_ops.append(("cx", [ancilla_idx + 2, qubits[2]]))
            clifford_ops.append(("h", [ancilla_idx + 2]))
    ...
    # If there are no Toffoli gates, the circuit is purely Clifford.
    if num_toffoli_gates == 0:
        final_stab = StabilizerStateChForm(num_qubits_orig)
        for gate, qubits in clifford_ops:
            _apply_gate_ch(final_stab, gate, qubits)
        return StabilizerDecomposedState(num_qubits_orig, [final_stab], [1.0 + 0j])

    # For circuits with Toffoli gates, construct the initial |Toffoli> state tensor product
    toffoli_tensor_state = construct_toffoli_tensor_state(num_toffoli_gates)

    final_stabilizers = []
    final_coeffs = []

    # Process each stabilizer component of the |Toffoli> state
    for stab, coeff in zip(toffoli_tensor_state.stabilizers, toffoli_tensor_state.coeffs):
        # Create the combined system: |0...0> (data) ⊗ |stab> (ancillas)
        full_stab_state = StabilizerStateChForm(num_qubits_orig).kron(stab)
        ...
        can_postselect_all = True
        prob_success = 1.0
        for i in range(num_toffoli_gates - 1, -1, -1):
            ancilla_qubits = [
                num_qubits_orig + 3 * i,
                num_qubits_orig + 3 * i + 1,
                num_qubits_orig + 3 * i + 2
            ]
            try:
                for q in ancilla_qubits:
                    p0 = full_stab_state.project_0(q)
                    prob_success *= p0
            except ValueError:
                can_postselect_all = False
                break
        ...
        if can_postselect_all:
            final_stab = full_stab_state
            for i in range(num_toffoli_gates - 1, -1, -1):
                final_stab = final_stab.remove_qubit(num_qubits_orig + 3 * i + 2)
                final_stab = final_stab.remove_qubit(num_qubits_orig + 3 * i + 1)
                final_stab = final_stab.remove_qubit(num_qubits_orig + 3 * i)

            final_stabilizers.append(final_stab)
            new_coeff = coeff * np.sqrt(prob_success * (4 ** num_toffoli_gates))
            final_coeffs.append(new_coeff)
